...
const annotationKey = `webhook.fujitsu.com/nginxserver`
...
// 2) check whether the pod contains a `webhook.fujitsu.com/nginxserver` annotation, if not return Allow directly.
// 3) change the image name
// 4) set the nodeSelector of the pod. This makes sure the pod can be scheduled to a node that support webhook.
func mutatePod(ar v1beta1.AdmissionReview) *v1beta1.AdmissionResponse {
	...
	for i, container := range newPod.Spec.Containers {
		// nginx is just an example.
		if strings.Contains(container.Image, "nginx") {
			newPod.Spec.Containers[i].Image = "nginx:1.14.0"
			newPod.Spec.Containers[i].Name = container.Name + "(mutated)"
		}
	}

	// set node selector:
	if newPod.Spec.NodeSelector != nil {
		newPod.Spec.NodeSelector["webhook"] = "true"
	} else {
		newPod.Spec.NodeSelector = map[string]string{"webhook": "true"}
	}
